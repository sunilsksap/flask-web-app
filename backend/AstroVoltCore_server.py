...
def export_quote_pdf(quote_id: int, display_number: str, host: str, port: int) -> Path:
    ...
    export_url = f"http://{host}:{port}/print/quotations/{quote_id}"
...
def salary_slip_html(row: sqlite3.Row) -> str:
...
<head>
  ...
  <style>
    ...
    .row {{
      ...
      gap: 10px;
      padding: 6px 0;
      border-bottom: 1px dashed #d5dcee;
    }}
    .row:last-child {{
      border-bottom: none;
    }}
...
<body>
  <div class="slip">
    <h1>AstroVoltCore Salary Slip</h1>
    <div class="meta">Month: {html.escape(row['month'])}</div>
    <div class="grid">
      <div>
        <div><strong>Employee</strong>: {html.escape(row['full_name'])}</div>
        <div><strong>Code</strong>: {html.escape(row['employee_code'])}</div>
        <div><strong>Department</strong>: {html.escape(row['department'] or '')}</div>
        <div><strong>Designation</strong>: {html.escape(row['designation'] or '')}</div>
      </div>
      <div>
        <div><strong>Bank</strong>: {html.escape(row['bank_name'] or '')}</div>
        <div><strong>Account</strong>: {html.escape(row['bank_account'] or '')}</div>
      </div>
    </div>

    <div class="row"><span>Base Salary</span><strong>{fmt(row['base_salary'])}</strong></div>
    <div class="row"><span>Allowances</span><strong>{fmt(row['allowances'])}</strong></div>
    <div class="row"><span>Overtime</span><strong>{fmt(row['overtime_amount'])}</strong></div>
    <div class="row"><span>LOP</span><strong>{fmt(row['lop_amount'])}</strong></div>
    <div class="row"><span>PF</span><strong>{fmt(row['pf_amount'])}</strong></div>
    <div class="row"><span>ESI</span><strong>{fmt(row['esi_amount'])}</strong></div>
    <div class="row"><span>Other Deductions</span><strong>{fmt(row['other_deductions'])}</strong></div>
    <div class="row"><span>Advance</span><strong>{fmt(row['advance'])}</strong></div>
    <div class="row total"><span>Net Pay</span><strong>{fmt(row['net_pay'])}</strong></div>
  </div>
</body>
</html>
""".strip()

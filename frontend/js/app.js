...
const FIXED_TERMS = [
  { title: "Validity", body: "This quotation is valid for 60 days from the date of issue." },
  ...
  { title: "Confidentiality", body: "The information contained in this quotation shall be confidential and shall not be disclosed to any third party without written consent." },
];
...
async function exportPdf() {
  ...
  try {
    ...
    if (!quoteId) {
      throw new Error("Save the quotation before generating the PDF.");
    }
    const exportUrl = `/api/quotations/${quoteId}/export-pdf?ts=${Date.now()}`;
    if (popup) {
      popup.location = exportUrl;
    } else {
      window.open(exportUrl, "_blank");
    }
    markSaved(`${state.current.display_number || "Quotation"} PDF generated.`);
  } catch (error) {
...
function renderList() {
  ...
  if (!records.length) {
    elements.quoteList.innerHTML = '<div class="empty-state">No quotations match the current search.</div>';
    return;
  }
...
Line Total: ${escapeHtml(formatMoney(itemTotalWithTax(item)))}</div>
  ...
  elements.dbRecordView.innerHTML = `
    <div class="db-block">
      <h3>Header</h3>
      <div class="db-pair"><div class="db-key">Quotation Number</div><div class="db-value">${escapeHtml(current.display_number || current.base_quote_number || "")}</div></div>
...
${escapeHtml(current.customer_gstin || "")}</div></div>
    ...
    <div class="db-block">
      <h3>Terms</h3>
      <div class="db-pair"><div class="db-key">Quote Date</div><div class="db-value">${escapeHtml(current.quote_date || "")}</div></div>
      <div class="db-pair"><div class="db-key">Valid Until</div><div class="db-value">${escapeHtml(current.valid_until || "")}</div></div>
    ...
    <div class="db-block">
      ...
      <div class="db-pair"><div class="db-key">Quoted Value in Words</div><div class="db-value">${escapeHtml(elements.amountWords.textContent || "")}</div></div>
    </div>
  `;
}
...
async function loadRecord(id) {
  ...
  state.selectedId = id;
  applyQuote(data.quotation);
  markSaved(`${data.display_number || data.quotation.display_number} loaded from database.`);
  renderList();
}

async function saveDraft() {
  const quotation = currentFormData();
  const data = await api("/api/quotations/save-draft", {
    method: "POST",
    body: JSON.stringify({ quotation }),
  });
  state.records = data.records || state.records;
  state.selectedId = data.record.id;
  applyQuote(data.record.quotation);
...
async function promptAndFinalize() {
  ...
  const pin = window.prompt("Enter the admin PIN to finalize and permanently lock this quotation.");
  if (!pin) {
    return;
  }
  const data = await api(`/api/quotations/${state.current.id}/finalize`, {
    method: "POST",
...
async function createRevision() {
  ...
  const data = await api(`/api/quotations/${state.current.id}/revise`, {
    method: "POST",
    body: JSON.stringify({ pin }),
  });
  state.records = data.records || state.records;
